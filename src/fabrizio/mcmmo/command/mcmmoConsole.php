    ...
    public function __construct(mcMMO $plugin, CommandSender $sender, array $args) {
        ...
        switch($args[0]){
            ...
            case "?":
                ...
                $sender->sendMessage("$prefix §4DANGER PLACE!!!");
                $sender->sendMessage("$prefix /mcmmo resetall | §7Reset all database.");
            ...
            case "top":
                ...
                foreach($array as $key => $values){
                    ...
                    $key = $username[$keys];
                    $sender->sendMessage("$prefix §f» §b{$i} §a{$key} §6Level: {$max}.");
                    unset($array[$keys]);
                    unset($username[$keys]);
                    if(sizeof($array) >0)
            ...
            break;
            case "resetall":
                $array = ["y1h33b", "yH30mv", "c3yaJ5", "aFb32h", "1hha37"];
                $rand = $array[array_rand($array)];
                $plugin->verification = "$rand";
                $sender->sendMessage("$prefix §4Type /mcmmo <verification code> to resetall.");
                $sender->sendMessage("$prefix §6Code§f: §a{$rand}");
                return true;
            break;
            case "skills":
                $array = [];
            ...
            case "check":
                ...
                $rank = $plugin->getRank($target, $skill);
                $targetxp = $level * 100;
            ...
            break;
            default:
                if($args[0] == $plugin->verification){
                    $plugin->database()->setAll([]);
                    $plugin->database()->save();
                    $sender->sendMessage("$prefix §4All database is clear.");
                    return true;
                }
